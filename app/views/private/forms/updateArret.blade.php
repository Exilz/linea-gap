...
<h1>Edition de l'arrêt "{{$arret->nomArret}}"</h1>

     {{Form::model($arret, array('class' => 'form'))}}
    
        <a href="delete/{{$arret->idArret}}" class="button button-delete delete right">Supprimer cet arret</a>
        ...
        @if($arret->accesArret == 1)
            {{Form::label('accesArret', "Oui")}}
            {{Form::radio('accesArret', '1', true )}}
            {{Form::label('accesArret', "Non")}}
            {{Form::radio('accesArret', '0' )}}
        @endif
        @if($arret->accesArret == 0)
            {{Form::label('accesArret', "Oui")}}
            {{Form::radio('accesArret', '1' )}}
            {{Form::label('accesArret', "Non")}}
            {{Form::radio('accesArret', '0', true )}}
        @endif
